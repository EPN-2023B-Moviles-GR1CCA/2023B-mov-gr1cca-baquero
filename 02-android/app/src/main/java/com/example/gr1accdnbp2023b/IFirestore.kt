...
class IFirestore : AppCompatActivity() {
    ...
    fun eliminarRegistro() {
        ...
        referenciaEjemploEstudiante.document("12345678").delete() // elimina
    ...
    fun guardarQuery(
        documentSnapshots: QuerySnapshot, refCities: Query
    ) {
    ...
    fun crearEjemplo() {
        ...
        val datosEstudiante = hashMapOf(
            ...
            "direccion" to hashMapOf(
                "direccion" to "Mitad del mundo", "numeroCalle" to 1234
            ),
        ...
        referenciaEjemploEstudiante // (crear/actualizar)
            .document(identificador.toString()).set(datosEstudiante).addOnSuccessListener { }
            .addOnFailureListener { }
        // Sin IDENTIFICADOR (crear)
        referenciaEjemploEstudiante.add(datosEstudiante).addOnCompleteListener { }
            .addOnFailureListener { }
    ...
    fun consultarIndiceCompuesto(adaptador: ArrayAdapter<ICities>) {
        ...
        limpiarArreglo()
        adaptador.notifyDataSetChanged()
        citiesRefUnico.whereEqualTo("capital", false).whereLessThanOrEqualTo("population", 4000000)
            .orderBy("population", Query.Direction.DESCENDING).get().addOnSuccessListener {
                for (ciudad in it) {
                    anadirAArregloCiudad(ciudad)
                }
                adaptador.notifyDataSetChanged()
